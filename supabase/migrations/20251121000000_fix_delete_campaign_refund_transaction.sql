-- 마이그레이션: delete_campaign 함수 수정 (refund 타입 campaign_id NULL 처리)
-- 날짜: 2025-11-21
-- 설명: 
--   1. refund 타입 트랜잭션 INSERT 시 campaign_id를 NULL로 설정하여 제약 조건 위반 방지
--   2. 제약 조건이 제대로 적용되었는지 확인 및 재적용

-- 1. 제약 조건 강제 재적용 (기존 제약 조건 제거 후 재생성)
-- transaction_type_check 제약 조건 재적용
...
-- 2. delete_campaign 함수 수정 (refund 타입 campaign_id NULL 처리)
...
BEGIN
  ...
  IF v_refund_amount > 0 THEN
    ...
    -- 캠페인 삭제 전이지만, 제약 조건 위반을 방지하기 위해 NULL로 설정
    -- description에 캠페인 정보를 포함하여 추적 가능하도록 함
    INSERT INTO public.point_transactions (
      wallet_id,
      transaction_type,
      amount,
      campaign_id, -- NULL로 설정하여 제약 조건 위반 방지
      description,
      created_by_user_id,
      created_at,
      completed_at
