-- Update campaigns table constraints and remove deprecated fields
-- Phase 0: 데이터 마이그레이션 (NULL 값 처리)
-- 테이블이 존재할 때만 실행
...
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'campaigns') THEN
    ...
    SET 
      ...
      platform = COALESCE(platform, 'coupang'),
      purchase_method = COALESCE(purchase_method, 'mobile'),
      review_type = COALESCE(review_type, 'star_only'),
      payment_method = COALESCE(payment_method, 'platform')
    WHERE 
    ...
    -- 2. 상품 정보 필드 NOT NULL 제약 조건 추가 (keyword 제외)
    ...
    -- 3. 리뷰 설정 필드 NOT NULL 제약 조건 추가
    ALTER TABLE public.campaigns
      ALTER COLUMN review_type SET NOT NULL;

    -- 4. 비용 정보 필드 NOT NULL 제약 조건 추가
    ALTER TABLE public.campaigns
      ALTER COLUMN payment_method SET NOT NULL;

    -- 5. 하위 호환성 필드 삭제 (인덱스 먼저 삭제)
...
-- 7. create_campaign_with_points_v2 함수 업데이트
