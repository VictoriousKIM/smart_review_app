-- 마이그레이션: 캠페인 생성 시 포인트 트랜잭션 누락 문제 수정
-- 문제: create_campaign_with_points_v2 함수에서 wallet UPDATE는 있지만 포인트 트랜잭션 INSERT가 없어서
--       포인트 트랜잭션 이력이 생성되지 않는 문제
-- 해결: 포인트 트랜잭션 INSERT 추가 (wallet UPDATE 후, 캠페인 생성 후 실행)
...
-- 주석 추가 (함수 시그니처를 정확히 지정)
COMMENT ON FUNCTION "public"."create_campaign_with_points_v2"(
  "p_title" "text",
  "p_description" "text",
  "p_campaign_type" "text",
  "p_campaign_reward" integer,
...
'캠페인 생성 시 포인트 차감 및 트랜잭션 기록을 원자적으로 처리합니다.
